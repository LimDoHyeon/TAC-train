...
class FaSNet_base(nn.Module):
    ...
    def pad_input(self, input, window):
        ...
        if rest > 0:
            pad = torch.zeros(batch_size, nmic, rest, dtype=input.dtype, device=input.device)
            input = torch.cat([input, pad], dim=2)
        pad_aux = torch.zeros(batch_size, nmic, stride, dtype=input.dtype, device=input.device)
    ...
    def seg_signal_context(self, x, window, context):
        ...
        # pad input accordingly (using pad_input)
        input, rest = self.pad_input(x, window)
        batch_size, nmic, nsample = input.shape
        stride = window // 2

        # pad another context size
        pad_context = torch.zeros(batch_size, nmic, context, dtype=input.dtype, device=input.device)
        input = torch.cat([pad_context, input, pad_context], dim=2)  # B, ch, L

        # calculate index for each chunk
        nchunk = 2 * nsample // window - 1
        begin_idx = np.arange(nchunk) * stride
        begin_idx = torch.from_numpy(begin_idx).to(input.device).type(input.dtype).long().view(1, 1, -1)  # 1, 1, nchunk
        begin_idx = begin_idx.expand(batch_size, nmic, nchunk)  # B, ch, nchunk

        # select entries from index
        chunks = [torch.gather(input, 2, begin_idx + i).unsqueeze(3) for i in range(2 * context + window)]
        chunks = torch.cat(chunks, dim=3)  # B, ch, nchunk, chunk_size
    ...
    def seq_cos_sim(self, ref, target):
        ...
        # L2 norms
        weight = torch.ones(ref.size(0) * ref.size(1), 1, target.size(2),
...
class FaSNet_TAC(FaSNet_base):
    ...
    def forward(self, input, num_mic):
        ...
        mic_context = torch.cat([all_mic_context.view(batch_size * nmic, 1, seq_length,
                                                      self.context * 2 + self.window)] * self.num_spk,
